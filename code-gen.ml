...
code_fragment:
 ...
 MALLOC (rax 1)
      ...
      mov	rbx, 0
      loop:
      mov qword[rdx] ,qword[rcx * 8] ;;get the j'th element in ext env
      mov qword[rdx] ,qword[rax+rdx] ;; get to its address from the start of the vector
      mov qword[rsi] ,qword[rbx * 8] ;;get the i'th element in env
      mov qword[rsi], qword[rsi +  rbp] ;; get to its address from the start of the vector -paz say that its rbp + 16
      mov qword[rsi] , qword[rsi + 16] 
 ...
 MALLOC rsp 8
    ...
    mov rbx, 1
  push rsi
  mov qword[rsi], qword[rbp + 32]
    push rdx
    mov rdx ,rbx  ;; save the original num of params
    loop1:
    mov qword[rcx] ,qword[8 * rbx]  ;;gets to the relevent i'th place insdide extenv[0][i]
    mov qword[rcx], qword[rsp+rcx] ;; the actual address of extenv[0][i]
    jmp find_param
